...
class AdherentDislikeController extends Controller
{
    use GlobalTrait;
    ...
    {
        try {
            $data = $this->likesRepository->deleteLike($userId, $offerId);

            return $this->globalResponse(trans('messages.SUCCESS'), Response::HTTP_OK, $data);
        } catch (Exception $exception) {
            Log::error('Add Dislike on offer Exception : ' . $exception->getMessage());
